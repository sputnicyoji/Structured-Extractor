...
description: "通用结构化信息提取专家。Use when: 需要从代码/文档/日志中提取
  ...
  (3) 5步后处理管道 (Source Grounding/去重/评分/消歧/关系推断)
  (4) 3种输出格式 (JSON文件/Markdown报告/知识图谱注入)。
  无需外部 API。"
---

# Structured Extractor

> 通用结构化信息提取 | 基于 Google LangExtract 算法层重构 | 5 步管道 + 3 种输出格式
...
4. **禁止低质量输出** - 输出时 confidence < 0.3 的提取必须过滤（无论输出格式）
...
我需要从文本中提取结构化信息?
    ...
    │   └─ 最小 → 仅 Scoring
    │
    └─ 输出格式? (Pipeline 完成后询问用户)
        ├─ JSON 文件   → 保存 pipeline 输出 (默认, 始终可用)
        ├─ Markdown 报告 → 生成人类可读的 .md 文件 (始终可用)
        └─ 知识图谱注入  → 调用 aim_create_entities (需要 KG MCP 工具)
```
...
Pipeline 完成后，**必须询问用户**想要什么输出格式。支持多选:

| 格式 | 说明 | 适用场景 | 依赖 |
|------|------|---------|------|
| **JSON 文件** | Pipeline 原始输出，保存为 `.json` | 程序消费、后续处理、存档 | 无 (始终可用) |
| **Markdown 报告** | 人类可读的结构化报告，保存为 `.md` | 代码审查、团队分享、文档补充 | 无 (始终可用) |
| **知识图谱注入** | 调用 `aim_create_entities` / `aim_create_relations` | 构建可检索的项目知识库 | 需要 KG MCP 工具 |

**询问方式** (使用 AskUserQuestion 或直接询问):

```
Pipeline 已完成。提取了 N 项结构化信息 (avg confidence: X.XX)。
请选择输出格式 (可多选):
1. JSON 文件 — 保存到指定路径
2. Markdown 报告 — 生成可读的 .md 文件
3. 知识图谱注入 — 写入 knowledge graph
```

#### Markdown 报告格式

```markdown
# Extraction Report: {source_file}

> Generated by structured-extractor | {date} | {total} extractions | avg confidence: {avg}
...
### [R1] {summary_cn} (confidence: {score})
...
### [C1] {summary_cn} (confidence: {score})
- **Source**: {source_file}:{line}
- **Check**: {attributes.condition_cn}
- **Code**: `{text}`

## Entities

### [E1] {summary_cn} (confidence: {score})
- **Source**: {source_file}:{line}
- **Kind**: {attributes.entity_kind}
- **Code**: `{text}`
...
| From | To | Type | Source |
|------|----|------|--------|
| {from} | {to} | {relation_type} | {source_file}:{line} |
```
...
| "用户只要 Markdown 报告" | 先 JSON 再转 Markdown，数据可复用 | 先 pipeline 后转格式 |
| "直接注入知识图谱就好" | 用户可能没有 KG 工具，必须先询问 | 询问输出格式 |
