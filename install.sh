...
EXPECTED_PATH=".cursor/skills/structured-extractor"

if [ "$RELATIVE_PATH" != "$EXPECTED_PATH" ]; then
    echo "[ERROR] Skill must be cloned to .cursor/skills/structured-extractor/"
    echo "  Expected: <project>/$EXPECTED_PATH"
    echo "  Actual:   $SKILL_ROOT"
    echo ""
    echo "Fix: git clone https://github.com/sputnicyoji/Structured-Extractor .cursor/skills/structured-extractor"
    exit 1
fi
...
MDC_SOURCE="$SKILL_ROOT/cursor/structured-extractor.mdc"
MDC_TARGET="$RULES_DIR/structured-extractor.mdc"
...
echo "[OK] Installed .mdc -> .cursor/rules/structured-extractor.mdc"
...
echo ""
echo "Skill root:  .cursor/skills/structured-extractor/"
echo "Cursor rule: .cursor/rules/structured-extractor.mdc"
echo ""
echo "Usage in Cursor: Ask the AI to extract structured information from any file."
